...
SELECT CategoryID, SUM(UnitsInStock) AS [tổng số lượng sản phẩm trong kho]
FROM dbo.Products
GROUP BY CategoryID
HAVING SUM(UnitsInStock) >350

-- Hãy cho biết những quốc gia nào có nhiều hơn 7 don hang.
SELECT ShipCountry, COUNT(OrderID) AS [Tổng đơn hàng]
FROM dbo.Orders
...
SELECT ShippedDate, COUNT(*) AS [totalShippedOrders]
FROM dbo.Orders
